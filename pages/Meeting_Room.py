...
#     # Check if chat history exists for the current user and meeting type
#     chat_exists = collection.find_one(
#         {"username": username, "meet_type": active_meeting_type, "chat": {"$exists": True, "$ne": []}}
#     )

#     # Display "End Chat" button only if chat history exists in the database
#     if chat_exists:
#         if st.button("End Chat"):
#             # Update the existing chat record to set chat_ended to True
#             collection.update_one(
#                 {"_id": chat_exists["_id"]},
#                 {"$set": {"chat_ended": True}}
#             )
